  ...
  function Cloth(leader){
    ...
    this.columnRenderer = new PhysicsRenderer( 
      8,
      shaders.simulationShaders.column,
      renderer
    );

    this.columnRenderer.setUniform( 'leader' , { 
      type:"v3" , 
      value:leader 
    });

    this.columnRenderer.addBoundTexture( this.physicsRenderer, 't_column' , 'output' );
    

    this.columnMat = new THREE.ShaderMaterial({

      uniforms:{ lookup: { type:"t" , value: null }},
      vertexShader: shaders.vertexShaders.column,
      fragmentShader: shaders.fragmentShaders.column

    });
    this.columnGeo = ParticleUtils.createLookupGeometry( 8 );
    this.column = new THREE.ParticleSystem( this.columnGeo , this.columnMat );
    scene.add( this.column );

    //var mesh = new THREE.Mesh( new THREE.CubeGeometry( 5 , 5 , 5) );
    var pTexture = this.createPosTexture( 8 );
    this.columnRenderer.reset( pTexture );

    this.columnRenderer.addBoundTexture( this.column , 'lookup' , 'output' );
    ...
    var tIri    = THREE.ImageUtils.loadTexture( '../img/iriLookup3.png' );
    ...
    console.log( 'HITS' );
    console.log( shaders.fragmentShaders.iri );
