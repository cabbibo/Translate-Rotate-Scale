...
void main(){
  ...
  if( vUv.x < size ){
    ...
    if( vUv.y < size ){
      ...
      force += springForce( leader , pos.xyz , 20. );

    }else{

      vec4 prevPosY = texture2D( t_pos , vec2( vUv.x , vUv.y - size ) );

      force += springForce( prevPosY.xyz , pos.xyz , 3. );
    ...
    vec4 columnPos = texture2D( t_pos , vec2( hSize , vUv.y ) );

    force += springForce( columnPos.xyz , pos.xyz , 10000. );



  }

  // Second column ( attracts to last column )
  if( vUv.x > size && vUv.x < 2. * size ){

    vec4 prevPosX = texture2D( t_pos , vec2( 1. - hSize , vUv.y ) );
    force += springForce( prevPosX.xyz , pos.xyz , 3. );

  }


  // Last column ( attracts to last column )
  if( vUv.x > 1. - size ){

    vec4 nextPosX = texture2D( t_pos , vec2( size + hSize , vUv.y ) );
    force += springForce( nextPosX.xyz , pos.xyz , 3. );
 ...
 /* float nextX = vUv.x + size;
  ...
  if( prevY < 0. ){
    ...
    vec4 prevPosY = texture2D( t_pos , vec2( vUv.x , prevY ) );
